...
DATA_PATH = '/home/nickfrosst/nam_data'
...
def load_telco_churn_data():
  ...
  Returns:
    ...
    input features `X` as a pandas.Dataframe and the labels `y` as a pd.Series.
  """
    df = read_dataset('telcom_churn.csv')
    train_cols = df.columns[1:-1]  # First column is an ID
    label = df.columns[-1]
    x_df = df[train_cols]
    # Impute missing values
    x_df['TotalCharges'] = x_df['total eve charge'].replace(' ', 0).astype('float64')
...
def load_mimic2_data():
  ...
  """
    ...
    df = read_dataset('mimic2/mimic2.data', header=None, names=column_names, delim_whitespace=True)
...
def load_recidivism_data():
  ...
  """
    ...
    df = read_dataset('recidivism/recid.data', header=None, names=column_names, delim_whitespace=True)
    train_cols = column_names[:-1]
    label = column_names[-1]
    x_df = df[train_cols]
    y_df = df[label]
...
def load_fico_score_data():
  ...
  """
    ...
    df = read_dataset('fico/fico_score.data', header=None, names=column_names, delim_whitespace=True)
    train_cols = column_names[:-1]
    label = column_names[-1]
    x_df = df[train_cols]
    y_df = df[label]
...
def get_train_test_fold(data_x, data_y, fold_num, num_folds, stratified=True, random_state=42):
...
def split_training_dataset(data_x, data_y, n_splits, stratified=True, test_size=0.125, random_state=1337):
